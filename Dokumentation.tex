...
In Abbildung \ref{fig:plan1} wird die Aufteilung der verschiedenen Aufgaben dargestellt. Der Client (oben) wird von Lars, die Datenbank mit SQL (rechts) wird von Jakob und der Server(Mitte), sowie die Verwaltung des File-Systems(links) wird von Robin programmiert und betreut.

%Bilder einbinden von der Planung!
...
Das Folgende Beispiel beinhaltet eine Tabelle mit zwei Zeilen und zwei Spalten. Tabellen in HTML werden Reihe für Reihe definiert. Eine Reihe beginnt mit \glqq tr\grqq{} und wird mit \glqq /tr\grqq{} beendet. Auf die selbe Art werden auch Tabelleneinträge, also die Spalten in den Zeilen, definiert.
